...
Disc.prototype.getImage = function () {
  var image = new Image();
  if (this.team === "human") { // Just use the ternary operator here, image.src = this.team === "human" ? 'blue' : 'orange'.
...
Disc.prototype.isCollidedWithWall = function (otherObject) {
  if ( // Just return here, no need for if.
...
Disc.prototype.isCollidedWithMovingObject = function (otherObject) {
  ...
  if (distance < radii) { // Just return here, no need for if.
    return true;
  }
  return false;
};
